...
import type { SiteContent, SectionId, Alignment } from './siteContent'
...
const alignClass = (a: Alignment) => a === 'center' ? 'text-center' : a === 'right' ? 'text-right' : 'text-left'
const alignItems = (a: Alignment) => a === 'center' ? 'items-center' : a === 'right' ? 'items-end' : 'items-start'

export default function LandingPage() {
  const [menuOpen, setMenuOpen] = useState(false)
  const [bookingOpen, setBookingOpen] = useState(false)
  const [bookingPlan, setBookingPlan] = useState<string | undefined>()
  ...
  const getSection = (id: SectionId) => content.sections.find(s => s.id === id)
  ...
  const renderSection = (id: SectionId) => {
    ...
    const al = sec.align
    const aClass = alignClass(al)
    const aItems = alignItems(al)

    switch (id) {
      case 'hero': return (
        <section key="hero" id="hero" className="relative min-h-screen flex flex-col justify-end pb-28 md:pb-20 px-5 md:px-12"
          ...
          <div className={`relative z-10 max-w-xl flex flex-col ${aItems} ${al === 'center' ? 'mx-auto' : al === 'right' ? 'ml-auto' : ''}`}>
            <motion.p className={`text-sm tracking-widest uppercase mb-3 ${aClass}`} style={{ color: accent }}
              initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              {content.hero.label}
            </motion.p>
            <motion.h2 className={`text-4xl md:text-6xl font-bold leading-tight mb-6 ${aClass}`} style={{ color: text }}
      ...
      case 'about': return (
        <section key="about" id="about" className={`px-5 md:px-12 py-16 md:py-24 max-w-5xl ${al === 'center' ? 'mx-auto' : ''}`}>
          <motion.h2 className={`text-3xl md:text-5xl font-bold leading-tight mb-8 ${aClass}`} style={{ color: text }}
            initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            {content.about.title}
          </motion.h2>
          <div className="md:grid md:grid-cols-2 md:gap-12">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
              <p className={`mb-4 font-semibold ${aClass}`} style={{ color: muted }}>{content.about.listTitle}</p>
              <ul className="space-y-2">
                {content.about.items.map((item) => (
                  <li key={item} className={`flex items-start gap-2 text-sm md:text-base ${al === 'center' ? 'justify-center' : al === 'right' ? 'justify-end' : ''}`} style={{ color: `${text}cc` }}>
            ...
            <motion.div className="mt-8 md:mt-0 flex flex-col justify-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.35 }}>
              <div className="rounded-2xl p-6 md:p-8 mb-6" style={{ backgroundColor: `${btn}18`, border: `1px solid ${btn}33` }}>
                <p className={`text-sm leading-relaxed ${aClass}`} style={{ color: `${text}bb` }}>
                  Каждая компания найдёт занятие по душе — от тихого кино до зажигательного корпоратива. Бронируй любой формат без ограничений.
                </p>
              </div>
              <button onClick={() => openBooking()}
                className={`px-8 py-4 rounded-full font-bold transition-all self-start ${al === 'center' ? 'self-center' : al === 'right' ? 'self-end' : ''}`}
                style={{ backgroundColor: btn, color: btnTxt }}>
      ...
      case 'features': return (
        <section key="features" id="features" className="px-5 md:px-12 py-16 md:py-24" style={{ backgroundColor: `${btn}18` }}>
          <motion.h2 className={`text-3xl md:text-4xl font-bold mb-10 ${aClass}`} style={{ color: text }}
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            {content.features.title}
          </motion.h2>
          <div className={`max-w-xl space-y-6 ${al === 'center' ? 'mx-auto' : al === 'right' ? 'ml-auto' : ''}`}>
            {content.features.items.map((item, i) => (
              <motion.div key={i} className={`flex items-start gap-4 ${al === 'center' ? 'justify-center' : al === 'right' ? 'justify-end' : ''}`}
                ...
                <p className={`pt-1.5 text-base ${aClass}`} style={{ color: `${text}cc` }}>{item}</p>
              </motion.div>
            ))}
          </div>
          <div className={`mt-10 ${aClass}`}>
            <button onClick={() => openBooking()}
      ...
      case 'pricing': return (
        <section key="pricing" id="pricing" className="px-5 md:px-12 py-16 md:py-24">
          <motion.h2 className={`text-3xl md:text-4xl font-bold mb-10 ${aClass}`} style={{ color: text }}
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
      ...
      case 'schedule': return (
        <section key="schedule" id="schedule" className="px-5 md:px-12 py-16 md:py-24" style={{ backgroundColor: `${btn}18` }}>
          <motion.h2 className={`text-3xl md:text-4xl font-bold mb-10 ${aClass}`} style={{ color: text }}
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            {content.schedule.title}
          </motion.h2>
          <div className={`max-w-2xl grid md:grid-cols-2 gap-8 ${al === 'center' ? 'mx-auto' : al === 'right' ? 'ml-auto' : ''}`}>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <p className={`font-bold mb-4 ${aClass}`} style={{ color: accent }}>{content.schedule.scheduleTitle}</p>
              <ul className="space-y-2 text-sm">
                {content.schedule.hours.map(d => (
                  <li key={d} className={`flex items-center gap-2 ${al === 'right' ? 'justify-end' : al === 'center' ? 'justify-center' : ''}`} style={{ color: `${text}bb` }}>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 }}>
              <p className={`font-bold mb-4 ${aClass}`} style={{ color: accent }}>{content.schedule.priceTitle}</p>
              <div className={`space-y-4 text-sm ${aClass}`} style={{ color: `${text}bb` }}>
      ...
      case 'contacts': return (
        <section key="contacts" id="contacts" className="px-5 md:px-12 py-16 md:py-24">
          <motion.h2 className={`text-3xl font-bold mb-8 ${aClass}`} style={{ color: text }}
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            {content.contacts.title}
          </motion.h2>
          <div className={`max-w-sm space-y-5 ${al === 'center' ? 'mx-auto' : al === 'right' ? 'ml-auto' : ''}`}>
            <div>
              <p className={`text-xs uppercase tracking-widest mb-1 ${aClass}`} style={{ color: accent }}>Телефон</p>
              <a href={`tel:${content.contacts.phone.replace(/\s/g, '')}`} className={`text-2xl font-bold block ${aClass}`} style={{ color: text }}>
                {content.contacts.phone}
              </a>
            </div>
            <div>
              <p className={`text-xs uppercase tracking-widest mb-1 ${aClass}`} style={{ color: accent }}>Адрес</p>
              <p className={aClass} style={{ color: `${text}bb` }}>{content.contacts.address}</p>
            </div>
            <div>
              <p className={`text-xs uppercase tracking-widest mb-3 ${aClass}`} style={{ color: accent }}>Мессенджеры</p>
              <div className={`flex gap-3 ${al === 'center' ? 'justify-center' : al === 'right' ? 'justify-end' : ''}`}>
  ...
  return (
    <div style={{ backgroundColor: bg, color: text, fontFamily: g.fontFamily }} className="min-h-screen">
      <BookingModal
        ...
        whatsappUrl={content.header.whatsappUrl}
      />
